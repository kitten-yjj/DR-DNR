...
from DNR.temporaryFiles import readLoad, readFile
...
# fileURL = "/home/yinjinjiao/pycharm_project/DNR/data/IEEE70/"
fileURL = "/Users/yinjinjiao/Documents/project/DNR/data/IEEE70/"
# 读取节点数据 L边数，data_node数据矩阵，node_con节点连接矩阵，r电阻矩阵
file_path = fileURL + "IEEE70-1.csv"
L, data_node, node_con, r_ij, x_ij = readFile.read_File(file_path, 70)
data_node = pd.read_csv(file_path).values

# 获取节点T1小时的负荷
data_load = readLoad.read_File(file_path, N)

# 获取分布式发电得历史数据
PG_samples = np.zeros((T1, N, w_no))
DG = [f"{fileURL}DG{i}-{w_no}.csv" for i in range(N)]
DG1 = [f"{fileURL}DG{i}-{50}.csv" for i in range(N)]
for t in range(T1):
    for i in range(N):
        if i in Sg:
...
def ds_solve(t_start, z0_ij):
    ...
    m.addConstr(N - 2 == gp.quicksum(z_ij[h] for h in range(L)), name="")
    for h in range(L):
        i, j = int(data_node[h][1]), int(data_node[h][2])
    ...
    for j in range(N):
        ...
        m.addConstr((0 == kl[69, j]), name="")

    for t1 in range(T1):
        m.addConstr(U[t1, 0] == 1, name="")
        m.addConstr(U[t1, 69] == 1, name="")
...
def ro_solve(t_start, z0_ij):
    ...
    for t1 in range(T1):
        for i in range(N):
            if i in Sg:
                m.addConstr(P_g[t1, i] <= pg_min[t1+t_start][i], name="")  # 加上t_start
                m.addConstr(P_g[t1, i] >= 0, name="")  # 加上t_start
                m.addConstr(Q_g[t1, i] == tan_x * P_g[t1, i], name="")

    for t1 in range(T1):# 加上t_start
        for i in range(N):
            m.addConstr(P_g[t1, i] <= pg_max[t1+t_start][i], name="")
            m.addConstr(P_g[t1, i] >= pg_min[t1+t_start][i], name="")
            m.addConstr(Q_g[t1, i] <= qg_max[t1 + t_start][i], name="")
    ...
    m.optimize()
    # m.write("model.lp")
    # print('Obj: %g' % obj.getValue(), P_g[0, 27].x, P_g[0, 47].x, P_g[0, 64].x)
