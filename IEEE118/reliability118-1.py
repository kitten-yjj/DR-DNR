...
def relity():
    ...
    # 抽取100个样本
    num_samples = 50
    samples = np.zeros((N, 24, num_samples))
    for t in range(24):
        samples[19][t] = monte_carlo_sampling(mean_value * weght[t], std_deviation, num_samples, 0, 4000)
        samples[73][t] = monte_carlo_sampling(mean_value * weght[t], std_deviation, num_samples, 0, 4000)
        samples[109][t] = monte_carlo_sampling(mean_value * weght[t], std_deviation, num_samples, 0, 4000)
    mean_value = 500  # 均值
    for t in range(24):
        samples[49][t] = monte_carlo_sampling(mean_value * weght[t], std_deviation, num_samples, 0, 4000)
        samples[95][t] = monte_carlo_sampling(mean_value * weght[t], std_deviation, num_samples, 0, 4000)
